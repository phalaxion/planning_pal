...
```apache
<VirtualHost *:80>
    ServerName poker.yourcompany.internal
...
```
/var/www/poker/          ← Apache serves this (frontend static files)
  index.html
  room.html
  style.css
  app.js

/opt/poker-server/       ← Go binary
  server
```
...
[Service]
ExecStart=/opt/poker-server/server
Restart=always
RestartSec=3
User=www-data
...
deploy:
    scp bin/server user@server:/opt/poker-server/server
    scp -r frontend/* user@server:/var/www/poker/
    ssh user@server "systemctl restart poker"
```
...
Build in this sequence to get something usable as fast as possible:

1. **Go skeleton** — `main.go` starts HTTP server, serves a placeholder HTML page on `/`
2. **Hub + Room + Client** — in-memory room management with goroutine-per-room pattern
3. **WebSocket handshake** — clients can connect and receive a `state_update` on join
4. **Voting** — clients can cast votes; server broadcasts masked state during voting phase
5. **Reveal** — facilitator reveals cards; server broadcasts unmasked state
6. **New round** — reset votes, archive to history, broadcast updated state
7. **Frontend lobby** — `index.html` create/join flow
8. **Frontend room** — `room.html` full table UI wired to WebSocket
9. **Reconnection** — client-side exponential backoff reconnect
10. **Polish** — voting history panel, copy room code, facilitator handoff on disconnect
